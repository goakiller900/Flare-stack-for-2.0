data:extend(
{
  {
    type = "item",
    name = "vent-stack",
    icon = "__Flare_Stack_Goa__/graphics/icon/vent-stack.png",
    icon_size = 32,
    subgroup = "production-machine",
    order = "e[chemical-plant]a",
    place_result = "vent-stack",
    stack_size = 50
  },
  {
    type = "item",
    name = "flare-stack",
    icon = "__Flare_Stack_Goa__/graphics/icon/flare-stack.png",
    icon_size = 32,
    subgroup = "production-machine",
    order = "e[chemical-plant]b",
    place_result = "flare-stack",
    stack_size = 50
  },
  {
    type = "item",
    name = "incinerator",
    icon = "__Flare_Stack_Goa__/graphics/icon/incinerator.png",
    icon_size = 32,
    subgroup = "production-machine",
    order = "e[chemical-plant]c",
    place_result = "incinerator",
    stack_size = 50
  },
  {
    type = "item",
    name = "electric-incinerator",
    icon = "__Flare_Stack_Goa__/graphics/icon/electric-incinerator.png",
    icon_size = 32,
    subgroup = "production-machine",
    order = "e[chemical-plant]d",
    place_result = "electric-incinerator",
    stack_size = 50
  }
})
